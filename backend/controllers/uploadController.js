...
const compressImage = async (inputPath, outputPath, mimetype, originalSize) => {
  try {
    ...
    if (originalSize < 50 * 1024) {
      return { success: false, reason: 'File too small to benefit from compression' };
    }
    ...
    if (!fs.existsSync(outputPath)) {
      return { success: false, reason: 'Compression failed - output file not created' };
    }
    
    const compressedSize = getFileSize(outputPath);
    if (compressedSize === 0) {
      return { success: false, reason: 'Compression failed - output file is empty' };
    }
...
const fileFilter = (req, file, cb) => {
  ...
  if (!allowedTypes.includes(file.mimetype)) {
    return cb(new Error(`Invalid file type: ${file.mimetype}. Only images, PDFs, and documents are allowed.`), false);
  }
  
  // Validate file name
  const filename = file.originalname;
  if (!filename || filename.length > 255) {
    return cb(new Error('Invalid filename. Must be between 1-255 characters.'), false);
  }
