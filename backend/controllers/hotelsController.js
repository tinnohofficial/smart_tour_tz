...
exports.createHotel = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      message:
        "Required fields missing: name, destination_id, description, capacity, and base_price_per_night are required",
    });
  }
  ...
  try {
    ...
    if (existingHotels.length > 0) {
      return res.status(400).json({
        message:
          "Hotel manager already has a hotel. Only one hotel is allowed per manager.",
      });
    }
    ...
    try {
      ...
      res.status(200).json({
        message:
          "Hotel created successfully. Your profile is now pending approval.",
      });
    } catch (error) {
...
exports.getManagerProfile = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch hotel profile", error: error.message });
  }
};
...
exports.updateManagerProfile = async (req, res) => {
  ...
  try {
    ...
    const [hotelRows] = await db.query("SELECT id FROM hotels WHERE id = ?", [
      userId,
    ]);
    ...
    if (
      capacity !== undefined &&
      (isNaN(capacity) || parseInt(capacity) <= 0)
    ) {
      return res.status(400).json({
        message: "Capacity must be a positive number",
      });
    ...
    if (
      ...
      (isNaN(base_price_per_night) || parseFloat(base_price_per_night) <= 0)
    ) {
      return res.status(400).json({
        message: "Base price per night must be a positive number",
      });
