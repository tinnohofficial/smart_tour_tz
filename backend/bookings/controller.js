...
exports.createBooking = async (req, res) => {
  const { transportId, hotelId, guideId, activityIds, startDate, endDate } = req.body;
  ...
  try {
    ...
    if (bookingRows.length === 0) {
      return res.status(404).json({ 
        message: "Booking not found or not in pending payment status or does not belong to you" 
      });
    }
    ...
    try {
      ...
      if (paymentMethod === 'card') {
        ...
        if (savingsRows.length === 0) {
          return res.status(400).json({ message: "You do not have a savings account" });
        }
      ...
      if (paymentSuccessful) {
        ...
        await connection.query(
          `INSERT INTO payments 
           (booking_id, user_id, amount, payment_method, status, transaction_ref) 
           VALUES (?, ?, ?, ?, 'successful', ?)`,
          [
            bookingId, 
            userId, 
        ...
        await connection.commit();
        res.status(200).json({ message: "Payment processed and booking confirmed" });
      } else {
      ...
      const [items] = await db.query(
        `SELECT bi.*, 
         CASE 
           ...
           WHEN bi.item_type = 'transport' THEN tr.origin || ' to ' || tr.destination
