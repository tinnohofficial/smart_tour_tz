...
const configureWalletAddress = async (req, res) => {
  try {
    ...
    const checkQuery = 'SELECT id FROM user_crypto_balances WHERE user_id = $1';
    const checkResult = await db.query(checkQuery, [userId]);
    
    if (checkResult.rows.length > 0) {
      // Update existing wallet
      await db.query(
        'UPDATE user_crypto_balances SET wallet_address = $1 WHERE user_id = $2',
        [walletAddress, userId]
      );
    } else {
      // Create new wallet entry (with 0 initial balance)
      await db.query(
        'INSERT INTO user_crypto_balances (user_id, wallet_address, crypto_balance) VALUES ($1, $2, 0)',
    ...
    res.status(500).json({ message: 'Failed to configure wallet address', error: error.message });
  }
};
...
const depositFiatFunds = async (req, res) => {
  try {
    ...
    const paymentQuery = 'SELECT id FROM user_payment_methods WHERE id = $1 AND user_id = $2';
    const paymentResult = await db.query(paymentQuery, [paymentMethodId, userId]);
    ...
    const accountQuery = 'SELECT id FROM savings_accounts WHERE user_id = $1';
    ...
    if (accountResult.rows.length === 0) {
      ...
      const newAccountResult = await db.query(
        'INSERT INTO savings_accounts (user_id, balance_usd, created_at) VALUES ($1, $2, NOW()) RETURNING id',
    ...
    const transactionResult = await db.query(
      `INSERT INTO savings_transactions 
       ...
       VALUES ($1, 'deposit', $2, $3, 'Fiat deposit to savings account')
       RETURNING id`,
    ...
    await db.query(
      `INSERT INTO payments 
       ...
       VALUES ($1, $2, 'card', $3, 'successful', $4)`,
      [userId, depositAmount, `DEPOSIT-${Date.now()}`, transactionResult.rows[0].id]
    );
    
    // Get current balance
    const balanceResult = await db.query(
      'SELECT balance_usd FROM savings_accounts WHERE user_id = $1',
      [userId]
    );
    ...
    res.status(200).json({
      ...
      newBalance: balanceResult.rows[0].balance_usd,
...
const depositCryptoFunds = async (req, res) => {
  try {
    ...
    const walletQuery = 'SELECT id, wallet_address FROM user_crypto_balances WHERE user_id = $1';
    ...
    const contractData = {
      ...
      params: {
        amount: depositAmount,
        token: tokenType || 'ETH',
        user: walletResult.rows[0].wallet_address
      }
    };
    ...
    const transactionResult = await db.query(
      `INSERT INTO savings_transactions 
       ...
       VALUES ($1, 'deposit', $2, 'Crypto deposit to savings', TRUE, $3)
       RETURNING id`,
    ...
    const balanceResult = await db.query(
      'SELECT crypto_balance FROM user_crypto_balances WHERE user_id = $1',
      [userId]
    );
    ...
    res.status(200).json({
      ...
      newBalance: balanceResult.rows[0].crypto_balance,
      transactionId: transactionResult.rows[0].id,
    ...
    res.status(500).json({ message: 'Failed to process crypto deposit', error: error.message });
  }
};
...
const getSavingsBalance = async (req, res) => {
  try {
    ...
    // Get fiat balance
    const fiatQuery = 'SELECT balance_usd FROM savings_accounts WHERE user_id = $1';
    const fiatResult = await db.query(fiatQuery, [userId]);
    const fiatBalance = fiatResult.rows.length > 0 ? fiatResult.rows[0].balance_usd : 0;
    
    // Get crypto balance
    const cryptoQuery = 'SELECT crypto_balance, wallet_address FROM user_crypto_balances WHERE user_id = $1';
    const cryptoResult = await db.query(cryptoQuery, [userId]);
    const cryptoBalance = cryptoResult.rows.length > 0 ? cryptoResult.rows[0].crypto_balance : 0;
    const walletAddress = cryptoResult.rows.length > 0 ? cryptoResult.rows[0].wallet_address : null;
    ...
    res.status(500).json({ message: 'Failed to get savings balance', error: error.message });
  }
};
