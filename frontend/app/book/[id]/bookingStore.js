...
export const useBookingStore = create((set, get) => ({
  ...
  skipOptions: {
    // Skip functionality for services
    skipTransport: false,
    skipHotel: false,
  },
  errors: {},
  ...
  fetchDestination: async (destinationId) => {
    ...
    return apiUtils.withLoadingAndError(
      async () => {
        const data =
          await destinationsService.getDestinationById(destinationId);
    ...
    return apiUtils.withLoadingAndError(
      async () => {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/hotels${destinationId ? `?destination_id=${destinationId}` : ""}`,
        );
        if (!response.ok) {
  ...
  createBooking: async () => {
    ...
    try {
      const result =
        await bookingCreationService.createFlexibleBooking(bookingData);
      return result;
    } catch (error) {
  ...
  setSelectedOrigin: (originId) =>
    set({ selectedOrigin: originId, errors: {} }),
  setSelectedTransportRoute: (routeId) =>
    set({ selectedTransportRoute: routeId, errors: {} }),
  ...
  setActivitySessions: (activityId, sessions) =>
    set((state) => ({
      activitySessions: {
        ...state.activitySessions,
        [activityId]: sessions,
  ...
  removeActivitySessions: (activityId) =>
    set((state) => {
      const newSessions = { ...state.activitySessions };
      delete newSessions[activityId];
    ...
    try {
      ...
      if (step === 1) {
        ...
        if (!skipOptions.skipTransport && !selectedTransportRoute) {
          newErrors.transportRoute =
            "Please select a transport route or skip transport";
        }
      } else if (step === 3) {
        ...
        if (selectedActivities.length === 0) {
          ...
          const missingSessions = selectedActivities.filter(
            (activityId) =>
              !activitySessions[activityId] || activitySessions[activityId] < 1,
          );
          if (missingSessions.length > 0) {
            newErrors.activitySessions =
              "Please specify number of sessions for all selected activities";
          }
        }
      } else if (step === 5) {
