  ...
  fetchDashboardData: async () => {
    return apiUtils.withLoadingAndError(
      async () => {
        ...
        try {
          pendingBookings = await api.get(
            "/api/bookings/hotel-bookings-pending",
          );
        } catch (error) {
        ...
        try {
          completedBookings = await api.get(
            "/api/bookings/hotel-bookings-completed",
          );
        } catch (error) {
        ...
        const recentBookings = completedBookings.slice(0, 5).map((booking) => ({
